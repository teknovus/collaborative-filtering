...
public class App {
    ...
    public static int NUM_DISHES = 1000;
...
/*
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            for (User testUser: testUsers) {
                User recommendation = generatePredictions(users, testUser.getUserNum());
    ...
    public static double vectorSimilarity(User user1, User user2) {
        double user1mag = 0.0;
        double user2mag = 0.0;
        double numerator = 0.0;
        for (int i = 0; i < NUM_DISHES; i++) {
            if(user1.getRatings().containsKey(i)) user1mag += Math.pow(user1.getRatings().get(i), 2);
            if(user2.getRatings().containsKey(i)) user2mag += Math.pow(user2.getRatings().get(i), 2);
            if(user1.getRatings().containsKey(i) && user2.getRatings().containsKey(i)) {
                numerator += user1.getRatings().get(i) * user2.getRatings().get(i);
            }
        }
        if (user1mag == 0 || user2mag == 0 || numerator == 0) return 0;
        return numerator / (Math.sqrt(user1mag) * Math.sqrt(user2mag));
    }

    public static User generatePredictions(ArrayList<User> users, int userNum) {
        double[] similarity = new double[users.size()];
        for(int i = 0; i < users.size(); i++) {
            similarity[i] = vectorSimilarity(users.get(userNum), users.get(i));
        }
        User predictions = new User();

        for(int dish = 0; dish < NUM_DISHES; dish++) {
            // if the user has not rated the dish, predict the rating
            if (!users.get(userNum).getRatings().containsKey(dish)) {
                double totalRating = 0.0; //of the dish
                int numRatings = 0;
                double numerator = 0.0;
                double denominator = 0.0;
                for(int otherUser = 0; otherUser < users.size(); otherUser++) {
                    if (otherUser == userNum) continue; //skip self
                    if (users.get(otherUser).getRatings().containsKey(dish)) {
                        numRatings++;
                        totalRating += users.get(otherUser).getRatings().get(dish);
                        // Similarity of other user times their rating for the dish
                        numerator += similarity[otherUser] * users.get(otherUser).getRatings().get(dish);
                        denominator += Math.abs(similarity[otherUser]);
                    }
                    double meanRating = totalRating / numRatings; // of dish by everyone
                }
                if (denominator == 0) predictions.getRatings().put(dish, users.get(userNum).getMeanRating()); // no similarity to any users, probably will not like
                else predictions.getRatings().put(dish, users.get(userNum).getMeanRating() + (numerator / denominator));
            }
        }
    ...
    /**
     * Restricted recommendations base to existing test data
     * @param user
     * @param test
     * @return
     */
